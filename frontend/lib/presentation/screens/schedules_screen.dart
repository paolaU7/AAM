...
class _HorariosScreenState extends State<HorariosScreen> {
  late final CourseRepositoryImpl _repo;
  ...
  void initState() {
    super.initState();
    _repo = CourseRepositoryImpl(_ds);
    _cargarCourses();
    _cargarTalleres();
  }

  Future<void> _cargarCourses() async {
    final cursos = await _repo.getCourses();
  ...
  List<String> get _anios {
    final s = _cursos.map((c) => c.academicYear).toSet().toList();
    s.sort();
    return s;
  }
  ...
  List<String> get _divisiones {
    ...
    final s = _cursos.where((c) => c.academicYear == _anioSel).map((c) => c.division).toSet().toList();
    s.sort();
    return s;
  }
      ...
      child: Row(children: [
        ...
        if (_cursoResuelto != null) ...
          [
            const SizedBox(width: 16),
            Text(
              '$_anioSel° $_divisionSel (${_cursoResuelto!.specialty})',
